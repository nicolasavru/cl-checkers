...
long getdatabasesize(int bm, int bk, int wm, int wk, int bmrank, int wmrank);
int choose(int n, int k);
int LSB(unsigned int x);
int MSB(unsigned int  x);
int recbitcount(unsigned int n);
int bitcount(unsigned int n);
int parseindexfile(char idxfilename[256],int blockoffset,int fpcount);
...
static unsigned int revword[65536];
...
int dblookup(POSITION *q,int cl)
    ...
    int i;
    unsigned int x,y;
    unsigned int bmindex=0,bkindex=0,wmindex=0,wkindex=0;
    unsigned int bmrange=1, wmrange=1, bkrange=1;
    int blocknumber;    
    int uniqueblockid;
    int reverse = 0;
...
int db_init(int suggestedMB, char out[256])
    ...
    for(i=0;i<65536;i++)
        bitsinword[i]=recbitcount((unsigned int)i);
